...
from android_tool.uiautomator import UINode, dump_ui_xml, parse_uiautomator_nodes, find_nodes
from android_tool.vision import find_template_on_screen_with_fallback
...
def _match_selector(node: UINode, selector: dict, case_sensitive: bool = False) -> bool:
    ...
    for key, val in selector.items():
        if key == "text":
            if not match_str(node.text, val, exact=True):
                return False
        elif key == "text_contains":
            if not match_str(node.text, val, exact=False):
                return False
        elif key == "desc":
