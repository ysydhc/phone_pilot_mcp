...
from android_tool.device_store import get_device_screenrecord_prefs, record_device_screenrecord_success
...
class WorkflowScreenRecorder:
    ...
    This is designed for `android_run_workflow(record_screen=True)`:
    ...
    def _run(self) -> None:
        ...
        # Keep original first, then progressively reduce bitrate/size.
        profiles: list[tuple[int, Optional[str]]] = []
        # Prefer a previously-successful profile from device store to avoid repeated failures.
        try:
            ...
            pass
        # Default probing order (works as fallback even without stored prefs).
        profiles += [
        ...
        while not self._stop.is_set():
            ...
            # If pull succeeded but produced an empty/tiny file, prune it to avoid confusion.
            # This usually happens when screenrecord fails to start the codec (rc!=0) but still
            # creates a zero-byte file on device.
            pruned_local = False
            try:
                local_size2 = None
                if isinstance(pull_res, dict):
                    local_size2 = pull_res.get("local_size")
                if isinstance(local_size2, int) and local_size2 >= 0 and local_size2 < 4096:
                    if local_path.exists():
                        local_path.unlink()
                        pruned_local = True
            except Exception:
            ...
            # Detect rapid failures (e.g. codec init errors) and stop endless looping.
            fail = False
            try:
                fail = (rc is None) or (int(rc) != 0)
            except Exception:
                fail = True
            # Also treat tiny outputs as failure.
            local_size = None
            if isinstance(pull_res, dict):
                local_size = pull_res.get("local_size")
            if isinstance(local_size, int) and local_size < 4096:
                fail = True

            # On success, record the working profile to device store for future runs.
            if not fail:
                try:
                    if self.device_serial and self.device_info_out_dir:
            ...
            if fail:
                consecutive_fail += 1
                # Switch profile after 2 consecutive failures.
                if consecutive_fail >= 2 and prof_i < len(profiles) - 1:
                    prof_i += 1
                    consecutive_fail = 0
                # Give up after exhausting profiles (or too many failures).
            ...
            seg += 1
            # If stop requested, do not start next segment.
            if self._stop.is_set():
                break
            # Small gap between segments to reduce flakiness.
            time.sleep(0.15)
