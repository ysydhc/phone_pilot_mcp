...
Usage:
    python covert_touch.py --input getevent.log --output touch.monkey
    adb shell getevent -lt | python covert_touch.py --output touch.monkey

# 中文注释:
# 本脚本将 `adb shell getevent -lt` 生成的触摸日志转换为 Monkey 的 `--scriptfile` 指令脚本。
# 仅支持单触点（默认 slot 0）流，并且通过解析 SYN_REPORT 边界来重建 DOWN/MOVE/UP 触摸操作，
# 最终以 Monkey 脚本的 DispatchPointer 和 UserWait 等命令输出，并且尽量还原事件间的时间延迟。
#
# 使用方法示例:
#     python covert_touch.py --input getevent.log --output touch.monkey
#     adb shell getevent -lt | python covert_touch.py --output touch.monkey
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    actions = scale_actions(actions, scale_x=float(args.scale_x), scale_y=float(args.scale_y), clamp=clamp)

    script_lines = to_monkey_script(actions)
