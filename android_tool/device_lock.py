...
from android_tool.android_device_utils import input_keyevent, input_swipe, get_screen_size
...
def get_device_state(device_serial: Optional[str]) -> dict:
    ...
    power_proc = CommandRunner.run(adb_prefix(device_serial) + ["shell", "dumpsys", "power"], check=False, log_output=False)
    ...
    policy_proc = CommandRunner.run(adb_prefix(device_serial) + ["shell", "dumpsys", "window", "policy"], check=False, log_output=False)
    ...
    # 3) focus window
    focus_proc = CommandRunner.run(adb_prefix(device_serial) + ["shell", "dumpsys", "window"], check=False, log_output=False)
    focus_line = ""
...
def wake_and_unlock(
    ...
    *,
    ...
    if wakeup:
        try:
            actions.append(input_keyevent(device_serial, "KEYCODE_WAKEUP", wait_s=0.0))
